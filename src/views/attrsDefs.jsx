...
export function DefsView() {
  ...
  const { rows: data = [], count = 0, totalPages: rawTotalPages = 1, error } = list;
  ...
  const reload = (f = applied) => {
    dispatch(attributes.defsListRequest({ params: buildParams(f) }));
  };
