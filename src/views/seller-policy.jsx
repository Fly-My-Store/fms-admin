...
import { getSellerPolicy, upsertSellerPolicy } from 'api/sellersStores';
...
export default function SellerPolicyView() {
  ...
  const load = async () => {
    try {
      if (!sellerId) return;
      const data = (await getSellerPolicy(sellerId)) || {};
      setForm({
  ...
  const save = async () => {
    try {
      if (!sellerId) return;
      await upsertSellerPolicy(sellerId, form);
    ...
    <>
      <Stack direction="row" spacing={2} sx={{ mb: 2 }}>
        ...
        <Stack alignItems="flex-end" justifyContent="flex-end" direction="row" spacing={2}>
          <Button variant="outlined" onClick={load} sx={{ mt: 'auto' }}>
            Load
          </Button>
          <Button variant="contained" onClick={save} sx={{ mt: 'auto' }}>
            Save
          </Button>
        </Stack>
      </Stack>
      ...
      <Stack spacing={2} sx={{ maxWidth: 720 }}>
        <Stack sx={{ gap: 1 }}>
          ...
          <TextField
            ...
            onChange={(e) => setForm((p) => ({ ...p, return_window_days: e.target.value }))}
            placeholder="e.g., 7"
          />
        </Stack>
        <Stack sx={{ gap: 1 }}>
        ...
        <Stack sx={{ gap: 1 }}>
          ...
          <TextField
            ...
            onChange={(e) => setForm((p) => ({ ...p, shipping_policy: e.target.value }))}
            placeholder="Text..."
          />
        </Stack>
      </Stack>
