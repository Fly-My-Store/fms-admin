...
const SETUP_STEPS = [
  ...
  { step: 9, label: 'Approve pending catalog', href: '/catalog-pending', note: 'Required after bulk import — items start Inactive' },
  { step: 10, label: 'Store listing (price + stock)', href: '/stores', note: 'Seller app, or admin Store → Variants CSV' },
  { step: 11, label: 'Fare / gateway (if needed)', href: '/fare' },
  { step: 12, label: 'Invoice settings', href: '/invoice-settings' }
];

function GuideTable({ headers, rows }) {
  return (
    <Box sx={{ overflowX: 'auto', mx: -0.5 }}>
      <Table size="small" sx={{ mt: 1.5, minWidth: 220, '& td, & th': { borderColor: 'divider' } }}>
        <TableHead>
          <TableRow>
            {headers.map((h) => (
...
function GuideButton({ href, children, ...props }) {
  const go = useGuideGo();
  return (
    <Button
      {...props}
...
function GuideTextLink({ href, children, sx, ...props }) {
  const go = useGuideGo();
  return (
    <Typography
      {...props}
      ...
      sx={{ color: 'primary.main', textDecoration: 'none', cursor: 'pointer', '&:hover': { textDecoration: 'underline' }, ...sx }}
    >
      {children}
    </Typography>
  );
...
export function AdminDataGuideContent() {
  return (
    <Stack spacing={2.5}>
      <Box>
        <Typography variant="body1" color="text.secondary">
          Use the same naming, structure, and pricing rules every time so apps, search, and checkout stay consistent. Open this guide from any screen (header document icon → right panel) or the full page. Expand a section when you need detail — start with Golden rules.
        </Typography>
      </Box>
      ...
      <Section title="Golden rules" defaultExpanded>
        ...
        <Box component="ol" sx={{ m: 0, pl: 2.5 }}>
          {[
            ...
            'Sellers list products — admin builds catalog; a store listing (price + stock) is what sells. Admin MRP alone does not sell.',
            'Bulk import creates Inactive rows — approve them in Pending catalog before customers can see them.',
            ...
            'Access is role-based — menu items and Save/Delete follow the user’s role (Create / Read / Modify / Delete). Super Admin always has full access.'
          ].map((rule) => (
        ...
        <Stack spacing={1}>
          {SETUP_STEPS.map((s) => (
            <Stack key={s.step} direction="row" alignItems="center" spacing={1.5} flexWrap="wrap">
              ...
              {s.href ? (
                <GuideTextLink href={s.href} variant="body2">
                  {s.label}
                </GuideTextLink>
              ) : (
...
Variant  →  Variant attributes (axes)
      ↓
Optional: Bulk import (lands Inactive) → Pending catalog (approve)
      ↓
Store listing (price + stock) — seller app or admin CSV update`}
        </Box>
      </Section>
      ...
      <Section title="Where things live">
        ...
        <DontList
          items={[
            'Name + role opens your profile.',
            'Document icon opens this guide as a right panel on any screen. Expand opens the full page (/admin-data-guide).',
            'Fullscreen toggles the browser window. Logout asks to confirm.'
          ]}
        />
        <Typography variant="subtitle2" sx={{ mt: 2, mb: 0.5 }}>
          Still reachable by URL (not in the sidebar)
        </Typography>
        <DontList
          items={[
      ...
      <Section title="1. Attribute definitions">
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
          ...
          <GuideTextLink href="/attribute-defs" variant="body2">
            Open Attribute defs
          </GuideTextLink>
        </Typography>
        <GuideTable
      ...
      <Section title="2. Brands">
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
          <GuideTextLink href="/brands" variant="body2">
            Open Brands
          </GuideTextLink>
        </Typography>
        <GuideTable
        ...
        <DontList
          items={[
            ...
            'Bulk-imported brands stay Inactive until approved in Pending catalog → Brands.'
          ]}
        />
        <Typography variant="subtitle2" sx={{ mt: 2, mb: 0.5 }}>
          What not to do
      ...
      <Section title="3. Categories & Screen Guard">
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
          <GuideTextLink href="/categories" variant="body2">
            Open Categories
          </GuideTextLink>
        </Typography>
      ...
      <Section title="4. Category attributes / axes">
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
          ...
          <GuideTextLink href="/category-attrs" variant="body2">
            Open Category attrs
          </GuideTextLink>
        </Typography>
        <GuideTable
      ...
      <Section title="5. Products & variants">
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
          <GuideTextLink href="/products" variant="body2">
            Open Products
          </GuideTextLink>
        </Typography>
        <GuideTable
      ...
      <Section title="7. Bulk import & pending catalog">
        <Alert severity="info" sx={{ mb: 1.5 }}>
          Bulk import creates catalog rows as Inactive. They do not go live until you approve them.
        </Alert>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
          <GuideTextLink href="/catalog-bulk-import" variant="body2">
            Open Bulk import
          </GuideTextLink>
          {' · '}
          <GuideTextLink href="/catalog-pending" variant="body2">
            Open Pending catalog
          </GuideTextLink>
        </Typography>
        <GuideTable
          headers={['Step', 'What']}
        ...
        <DontList
          items={[
            'Do not skip Pending catalog after an import — Inactive items stay hidden in apps.',
            'Do not re-import the same SKUs to “fix” a failed row; fix the CSV and retry only failed rows.',
            'Do not approve a product while its brand, category, or images are still pending.'
          ]}
        />
      </Section>

      <Section title="8. Seller listing & store variant CSV">
        <Alert severity="info" sx={{ mb: 1.5 }}>
        ...
        <GuideTable
          ...
          rows={[
            ['Catalog', 'Admin', 'Brand, category, product, variants, attrs, images — Active (or approved from pending)'],
            ['Listing', 'Seller app or admin', 'Store + variant → price + stock + listing Active'],
            ['SG permission', 'Admin on store/seller', 'Enable can_sell_screen_guard for Screen Guard sellers']
          ]}
        />
        <Typography variant="subtitle2" sx={{ mt: 2 }}>
          Admin bulk update (existing listings only)
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
          On a live store, open the <strong>Variants</strong> tab.{' '}
          <GuideTextLink href="/stores" variant="body2">
            Open Stores
          </GuideTextLink>
        </Typography>
        <GuideTable
        ...
        <DontList
          items={[
            'After creating a new variant, someone must list that SKU — old listings do not auto-cover new variants. CSV update will not add missing SKUs.',
            'Do not drop or rewrite the id column. A new id, or an id from another store, fails that row.',
      ...
      <Section title="9. Fare and pricing">
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
          ...
          <GuideTextLink href="/fare" variant="body2">
            Open Fare
          </GuideTextLink>
        </Typography>
        <GuideTable
        ...
        <Typography variant="body2" sx={{ mt: 1.5 }}>
          ...
          <GuideTextLink href="/invoice-settings" variant="body2">
            /invoice-settings
          </GuideTextLink>
      ...
      <Section title="10. Images & uploads">
        <GuideTable
          headers={['Rule', 'Detail']}
          rows={[
      ...
      <Section title="11. Record status">
        <GuideTable
      ...
      <Section title="12. Common problems → what to check">
        <GuideTable
          ...
          rows={[
            ...
            ['Imported items not in app', 'Pending catalog — still Inactive? Approve brand + category + product + variant + images.'],
            ['Listing CSV “not found”', 'id must be this store’s listing UUID. Do not create new rows; download from Variants, edit, reupload.'],
            ['CSV update ignored SKU', 'sku/product_name are labels. Only id + price/mrp/stock/max fields are written.'],
            ['Menu item missing', 'Role lacks Read on that resource. Super Admin bypasses. Hidden URLs (carts, PLP, etc.) are not in the sidebar on purpose.'],
            ['Cannot approve pending', 'Need Modify on brand/category/product. Tabs you cannot modify are hidden.'],
            ['Cannot Save / Delete', 'Role needs Create / Modify / Delete on that resource — Read-only roles can open the page but not change it.']
          ]}
        />
      </Section>

      <Section title="13. Quick checklist before Save">
      ...
      <Section title="14. Roles & permissions">
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
          Each admin user needs a role. The role is a matrix of resources (Orders, Brands, Stores, …) × Create / Read / Modify / Delete.{' '}
          <GuideTextLink href="/roles" variant="body2">
            Open Roles
          </GuideTextLink>
          {' · '}
          <GuideTextLink href="/users" variant="body2">
            Open Users
          </GuideTextLink>
        </Typography>
        <GuideTable
          headers={['Rule', 'Detail']}
          rows={[
            ['Super Admin', 'Role code SUPER_ADMIN always has full access. Do not strip that role from the last super user.'],
            ['New user', 'Role is required on create. A user with no role cannot use gated screens.'],
            ['Read', 'See the menu item and open the page / list.'],
            ['Create', 'Add button and create APIs (including catalog bulk import).'],
            ['Modify', 'Edit, approve pending catalog, fare, store listing CSV update.'],
            ['Delete', 'Remove / purge.'],
            ['Permission catalog', 'Names are the resource keys the app uses. Do not rename live rows to “fix labels” — edit the description instead.']
          ]}
        />
        <DontList
          items={[
            'If someone cannot see Catalog → Pending, grant Modify on brand, category, or product — not a new menu item.',
            'Demo/tester admins write to demo data only (green header banner). Listing CSV update is download-only on demo stores.'
          ]}
        />
      </Section>
      ...
      <Section title="16. When to call engineering">
        <DontList
          items={[
            ...
            'Renaming permission catalog keys or changing how CRUD maps to APIs'
          ]}
        />
      </Section>
