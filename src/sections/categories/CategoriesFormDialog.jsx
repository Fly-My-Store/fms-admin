...
export default function CategoriesFormDialog({ open, onClose, initialData = null }) {
  const dispatch = useDispatch();
  const [form, setForm] = useState({ name: '', slug: '', parent_id: '' });

  useEffect(() => {
    if (initialData) {
      setForm({ ...{ name: '', slug: '', parent_id: '' }, ...initialData });
    } else {
  ...
  const handleSubmit = () => {
    if (initialData?.id) {
      dispatch(catalog.categoriesUpdateRequest({ params: { id: initialData.id, data: form } }));
    } else {
      dispatch(catalog.categoriesCreateRequest({ params: form }));
    }
    onClose();
  };
      ...
      <DialogContent>
        <Stack spacing={2} mt={1} minWidth='400px'>
          ...
          <Stack sx={{gap: 1}}>
            ...
            <TextField id="parent_id" name="parent_id" type="text" value={form.parent_id || ''} onChange={handleChange} placeholder="Parent ID" fullWidth />
          </Stack>
