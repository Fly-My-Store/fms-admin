...
export default function RiderPayoutsCard({ riderId, riderName, onPayoutRecorded }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [summaryRes, listRes] = await Promise.all([
        getPayoutSummary({ payee_type: 'RIDER', payee_id: riderId }),
        listPayouts({ payee_type: 'RIDER', payee_id: riderId, limit: 20 })
      ]);
  ...
  const payee = {
    payee_type: 'RIDER',
    payee_id: riderId || '',
    payee_name: riderName || wallet?.payee_name || ''
  };
      ...
      >
        ...
        {!loading && !error && (
          <Stack spacing={2}>
            ...
            {!hasWalletActivity && (
              <Alert severity="info">
                No earnings recorded yet. Rider wallet balances update after completed deliveries with captured payments.
              </Alert>
            )}
            {wallet?.payout_hint && (
