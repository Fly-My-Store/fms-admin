...
import { getZones } from 'api/location';
...
export default function ZoneSelector({ value, onChange, multiple = false, textFieldProps = {} }) {
  const fetchOptions = async (search, page) => {
    const response = await getZones({ search, page, limit: 50 })
