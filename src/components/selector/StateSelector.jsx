...
import { getStates } from 'api/location';
...
export default function StateSelector({ value, onChange, multiple = false, textFieldProps = {} }) {
  const fetchOptions = async (search, page) => {
    const response = await getStates({ search, page, limit: 50 })
